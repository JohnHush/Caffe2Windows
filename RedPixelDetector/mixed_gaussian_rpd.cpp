#include "mixed_gaussian_rpd.hpp"
#include <iostream>

void MixedGaussianRPD::deNoise( IplImage * imgRst )
{
    ...
    for ( int irow = 0 ; irow < img_->height ; ++ irow )
    {
        ...
        for ( int icol = 0 ; icol < img_->width ; ++ icol )
        {
            ...
            float feature1 = ( CRed + epsilon_ ) / ( CGre + epsilon_ ) ; 
            float feature2 = ( CRed + epsilon_ ) / ( CBlu + epsilon_ ) ; 

            pair<float , float> tmp = make_pair( feature1 , feature2 );

            if ( !RedPtEx_.isGray(tmp) )
            {
                pt_count ++;
...
void MixedGaussianRPD::getRedPixels( IplImage * imgRst )
{
	cvSetZero( imgRst );
	if ( !hasRed_ )
		return;
...
void MixedGaussianRPD::hasRedPixels()
{
	...
	for ( int irow = 0 ; irow < img_->height ; ++ irow )
    {
        ...
        for ( int icol = 0 ; icol < img_->width ; ++ icol )
        {
            ...
            features.push_back( make_pair( (CRed+epsilon_) / (CGre+epsilon_), (CRed+epsilon_) / (CBlu+epsilon_)) );
        }
    }
	// get all the feature pts in img_
	...
	RedPtEx_.initExtractor( features );
	RedPtEx_.EMAlgorithm();
	/*
     * train a RedPixelsExtractor to classify the data points
     */
	float LIKELIHOOD_2CLASS = 0., LIKELIHOOD_1CLASS = 0.;

    LIKELIHOOD_2CLASS = RedPtEx_.relativeLikelihood( MODEL_PRIOR_2CLASS_ );
    ...
    {
        LIKELIHOOD_1CLASS += log( MODEL_PRIOR_1CLASS_ * prior_pro_2d(features[iDATA] , exp_1class , cov_1class ) );
    }
	if ( LIKELIHOOD_2CLASS > LIKELIHOOD_1CLASS )
		hasRed_ = true;
	else
		hasRed_ = false;
//	std::cout << "likelihood_111class = " << LIKELIHOOD_1CLASS << std::endl;
//	std::cout << "likelihood_222class = " << LIKELIHOOD_2CLASS << std::endl;
}
